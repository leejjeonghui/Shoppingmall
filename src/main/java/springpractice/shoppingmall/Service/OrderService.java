...
public class OrderService {
    ...
    public ResponseEntity<OrderDetailResponseDto> findOrder(Long orderId) {
        ...
        OrderDetailResponseDto orderResponse = new OrderDetailResponseDto(order.getId(), orderProduct, getTotalPrice(order.getId()));
    ...
    public ResponseEntity<String> cancleOrder(Long id) {
        Order order = orderRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("해당주문이 존재하지 않음"));
        order.deleteOrder();
